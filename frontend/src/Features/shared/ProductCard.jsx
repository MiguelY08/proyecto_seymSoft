...
function ProductCard({ image, name, category, price, productId, onAddToCart, onAddToFavorites }) {
  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-gray-200 transition-all duration-200 hover:shadow-lg hover:border-gray-300 overflow-hidden active:scale-95 active:shadow-md cursor-pointer">
      <Link to={`/${productId || 'shop/detail'}`}>
        ...
        <div className="p-2 sm:p-3 flex flex-col gap-1 sm:gap-1.5">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button
              ...
              className="w-7 h-7 sm:w-9 sm:h-9 lg:w-10 lg:h-10 flex items-center justify-center cursor-pointer shrink-0"
            >
              {isFavorite ? (
                isHoveringHeart ? (
