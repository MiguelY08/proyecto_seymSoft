...
import { Search, Home, Store, Package, Heart, ShoppingCart, User, Menu } from 'lucide-react';
...
function Header() {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 border-b border-gray-200 shadow-sm transition-all duration-150 ${
        scrolled 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`flex items-center justify-between gap-3 sm:gap-6 transition-all duration-150 ${
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              <div className={`rounded-full overflow-hidden cursor-pointer transition-all duration-150 ${
              ...
              <h1 className={`font-serif italic text-blue-900 font-semibold tracking-tight hidden md:block transition-all duration-150 ${
            ...
            <div className="flex-1 max-w-xs sm:max-w-md lg:max-w-2xl">
              <div className="relative group">
                ...
                <button className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 cursor-pointer">
                  <Search className={`text-gray-500 hover:text-blue-600 transition-all duration-150 ${
            ...
            <nav className="hidden lg:flex items-center gap-1">
              <NavLink icon={Home} label="Inicio" active={activeNav === 'Inicio'} onClick={() => setActiveNav('Inicio')} scrolled={scrolled} />
              <NavLink icon={Store} label="Tienda" active={activeNav === 'Tienda'} onClick={() => setActiveNav('Tienda')} scrolled={scrolled} />
              <NavLink icon={Package} label="Pedidos" active={activeNav === 'Pedidos'} onClick={() => setActiveNav('Pedidos')} scrolled={scrolled} />
            </nav>

            {/* Action Icons */}
            <div className="flex items-center gap-1 sm:gap-2">
              <IconButton icon={Heart} badge={8} className="hidden sm:block" scrolled={scrolled} />
              <IconButton icon={ShoppingCart} badge={0} scrolled={scrolled} />
              <IconButton icon={User} className="hidden sm:block" scrolled={scrolled} />
              ...
              >
                <Menu className={`text-gray-700 transition-all duration-150 ${
          ...
          {menuOpen && (
            <div className="lg:hidden border-t border-gray-200 py-3">
              <nav className="flex flex-col gap-2">
                <NavLinkMobile icon={Home} label="Inicio" active={activeNav === 'Inicio'} onClick={() => { setActiveNav('Inicio'); setMenuOpen(false); }} />
                <NavLinkMobile icon={Store} label="Tienda" active={activeNav === 'Tienda'} onClick={() => { setActiveNav('Tienda'); setMenuOpen(false); }} />
                <NavLinkMobile icon={Package} label="Pedidos" active={activeNav === 'Pedidos'} onClick={() => { setActiveNav('Pedidos'); setMenuOpen(false); }} />
              </nav>
      ...
      <div className={`transition-all duration-150 ${scrolled ? 'h-12 sm:h-14' : 'h-16 sm:h-20'}`}></div>
    </>
  );
};
...
const IconButton = ({ icon: Icon, badge, className = '', scrolled}) => {
  return (
    <button className={`relative rounded-full hover:bg-blue-50 transition-all duration-150 group cursor-pointer ${
      scrolled ? 'p-1.5 sm:p-2' : 'p-2 sm:p-2.5'
    } ${className}`}>
      <Icon className={`text-gray-700 group-hover:text-blue-600 transition-all duration-150 ${
        scrolled ? 'w-4 h-4' : 'w-5 h-5'
      }`} strokeWidth={2} />
      {badge !== undefined && badge > 0 && (
        <span className={`absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-md ring-2 ring-white transition-all duration-150 ${
