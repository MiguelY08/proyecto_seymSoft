...
function Evidence({ isOpen, onClose, onSave, files = [], descripcion = '' }) {
  ...
  // Abre el archivo real del computador en una nueva pestaña
  const openFile = (file) => {
  ...
  const handleSave = () => {
    onSave?.({ files: localFiles, descripcion: localDesc });
    onClose?.();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[480px] overflow-hidden">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          {/* Lista de archivos — clic en nombre/ícono abre el archivo real */}
          {localFiles.length > 0 && (
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              {localFiles.map((file, i) => (
                <div key={i}
                  ...
                  {/* Ícono con check verde — clickeable */}
          ...
          <div>
            ...
            <textarea value={localDesc} onChange={(e) => setLocalDesc(e.target.value)}
              placeholder="Agrega una breve explicacion" rows={4}
