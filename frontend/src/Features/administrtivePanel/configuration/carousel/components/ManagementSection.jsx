...
function ManagementSection({ slides, onAdd, onDelete, onToggle, loading }) {
  ...
  const expandedSlide =
    expandedId !== null ? slides.find((slide) => slide.id === expandedId) : null;
  ...
  const handleAdd = async (file) => {
    ...
    showSuccess(
      'Imagen agregada',
      'La imagen fue agregada al carrusel exitosamente.'
    );
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (slide?.activo) {
      showWarning(
        ...
        'Primero debes desactivar la imagen antes de eliminarla.'
      );
      return;
    }
  ...
  const handleToggle = async (id) => {
    ...
    try {
      ...
      showError('Error', 'No se pudo actualizar el estado de la imagen.');
    }
  };
  ...
  return (
    <>
      ...
      {expandedSlide && (
        <Lightbox
          imageUrl={expandedSlide.imageUrl}
