...
export default function PermissionsGrid({
  ...
  const toggleModuloCompleto = (moduloId) => {
    ...
    const updated = permisosRol.map((modulo) => {
      if (modulo.id === moduloId) {
        const allSelected = Object.values(modulo.acciones).every(Boolean);
  ...
  const toggleAllModules = () => {
    ...
    const updated = permisosRol.map((modulo) => {
      const nuevasAcciones = Object.keys(modulo.acciones).reduce((acc, key) => {
        acc[key] = !allSelected;
        return acc;
      }, {});
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {permisosSistema.map((modulo) => {
          ...
          const hasPermission =
            rolModulo && Object.values(rolModulo.acciones).some(Boolean);

          const allChecked =
            rolModulo && Object.values(rolModulo.acciones).every(Boolean);
          ...
          return (
            ...
            >
              ...
              <div className="flex justify-between items-center mb-3">
                <h4 className="font-semibold text-sm">{modulo.modulo}</h4>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {modulo.acciones.map((accion) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={rolModulo?.acciones?.[accion] || false}
                      onChange={() => toggleAccion(modulo.id, accion)}
                      className={`accent-blue-600 ${
                        readOnly
