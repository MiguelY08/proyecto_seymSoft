...
function Shop() {
  ...
  const handleCategoryChange = (category) => {
    ...
    if (updated.includes(category)) {
      ...
      const parent = categoriesBase.find(cat => cat.subcategories.includes(category));
      if (parent && !updated.includes(parent.name)) updated.push(parent.name);
    }
  ...
  const filteredProducts = products.filter(product => {
    const mainCategories = selectedCategories.filter(cat => categoriesBase.some(c => c.name === cat));
    const subCategories  = selectedCategories.filter(cat => categoriesBase.some(c => c.subcategories.includes(cat)));
    const matchMain  = mainCategories.length === 0 || mainCategories.includes(product.category);
    const matchSub   = subCategories.length  === 0 || subCategories.includes(product.subcategory);
  ...
  const handleToggleFavorite = (product) => {
    ...
    // Al quitar desde la tienda no se muestra confirmación (solo desde Favorites)
  };
            ...
            {currentProducts.length === 0 ? (
              <div className="w-full flex justify-center items-center py-20">
                <div className="bg-yellow-50 border border-yellow-300 text-yellow-700 px-6 py-5 rounded-lg text-center max-w-md">
                  ...
                  <p className="text-sm">Intenta cambiar los filtros o seleccionar otra categoría.</p>
                </div>
              </div>
            ) : (
